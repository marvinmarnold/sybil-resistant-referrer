...
import { MainChainContract } from "./LayerZeroMainChain.sol";

//@notice A Referral Campaign Contract for storing referres and their respective referees
contract CrossReferralCampaign is Ownable, Initializable {
    ...
    /// @dev Campaign token Chain ID
    uint16 campaignTokenChainId;

    MainChainContract tokenBalanceContract = MainChainContract(0xFeC9AD52dA5eCC6e7b18225Ec517F57276BFBFdF);
    ...
    // @dev Mapping Current Chain ID to respective tokenBalanceContractAddresses

    mapping(uint256=>address) chainToTokenBalance;
    ...
    function initialize (
        ...
        worldId = _worldId;
        // campaignTokenChainId=_campaignTokenChainId;
    }
    ...
    function getTokenBalance() public view returns(uint tokenBalance){
        return tokenBalanceContract.getBalance(msg.sender,campaignTokenContract,campaignTokenChainId);
    }


    function setChainId(uint16 _chainId,address _tokenBalanceContract) public {
        campaignTokenChainId=_chainId;
        chainToTokenBalance[_chainId]=_tokenBalanceContract;
    }
    ...
    function acceptReferral(address _referrer, address signal, uint256 root, uint256 nullifierHash, uint256[8] calldata proof) public {
        ...
        uint256 senderBalance = tokenBalanceContract.getBalance(msg.sender,campaignTokenContract,campaignTokenChainId);
