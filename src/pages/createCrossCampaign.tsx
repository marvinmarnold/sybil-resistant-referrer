import { Box, Button, FormControl, FormHelperText, FormLabel, Heading, Input, Menu, MenuButton, MenuItem, MenuList, Text, useBreakpointValue, useColorMode, useToast } from '@chakra-ui/react'
...
const createCrossCampaign = () => {
 ...
 const { config:SecondConfig, error:SecontxError, isError:SecondIsError } = usePrepareContractWrite({
     ...
     {isConnected && (
      <div
       style={{
        ...
        {SecondWriteSuccess&&<SuccessComponent
         link={'/createlink'}
         data={SecondData}
       ...
       <Heading as="h2" fontSize="32px" fontFamily="Dm Sans" textAlign="center" marginBottom="6px">
         ...
         {selectedChain && (
          <Text
            ...
            textAlign="center"
            variant="outline"
          >{`Chain ID: ${chainOptions.find((chain) => chain.name === selectedChain)?.chainId}`}</Text>
        )}
        <Menu>
          <MenuButton
            as={Button}
        ...
        {showButton && (
        <Button mt={4} backgroundColor="purple.300" marginLeft={4} marginBottom={2}
        variant="gradient"
        border={'0.5px solid #312E2A'}
        boxShadow={'2.8px 3.8px 0px 0px #312E2A'} 
        fontFamily="Dm Sans"
      ...
      {!writeSuccess && !SecondWriteSuccess &&  <form>
